...
@router.post(
    ...
    responses={
        201: {"description": "Prediction created successfully"},
        400: {"description": "Invalid request data"},
    },
)
def add_prediction(prediction: PredictionCreate, db: Session = Depends(get_db)):
...
def generate_ml_prediction(data: MLPredictionRequest, db: Session = Depends(get_db)):
    ...
    if result["prediction"] == 1:
        ...
        recommendation = "Inspect robot operating conditions and joint parameters."
    else:
        predicted_fault = "No Protective Stop"
        recommendation = "Robot operating conditions appear normal."
...
def generate_robot_prediction(robot_id: int, db: Session = Depends(get_db)):
    prediction = predict_for_robot(db, robot_id)

    if not prediction:
        raise HTTPException(status_code=404, detail="No telemetry found for this robot")
...
@router.get(
    ...
    responses={200: {"description": "Predictions retrieved successfully"}},
)
...
def fetch_prediction(prediction_id: int, db: Session = Depends(get_db)):
...
@router.put(
    ...
    responses={
        200: {"description": "Prediction updated successfully"},
        400: {"description": "Invalid request data"},
        404: {"description": "Prediction not found"},
    },
)
def edit_prediction(
    prediction_id: int, prediction: PredictionUpdate, db: Session = Depends(get_db)
):
