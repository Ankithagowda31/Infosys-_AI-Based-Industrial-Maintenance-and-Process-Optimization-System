...
def show_login():
    ...
    with col_center:
        ...
        if st.session_state["auth_mode"] == "signin":
                ...
                if submit_login:
                    if not email or not password:
                        ...
                        if data and "access_token" in data:
                            ...
                            st.error(
                                f"Sign in failed: {err or 'Invalid email or password'}"
                            )
            ...
            if st.button(
                "Create Account", key="switch_to_register", use_container_width=True
            ):
            ...
            with st.form("apple_register_form"):
                ...
                if submit_register:
                    if not reg_name or not reg_email or not reg_pwd:
                        ...
                        if reg_data:
                            st.success(
                                f"Account created for {reg_data.get('full_name')}. You can now sign in."
                            )
                            st.session_state["auth_mode"] = "signin"
                            st.rerun()
                        else:
                            st.error(
                                f"Registration failed: {reg_err or 'Unable to complete registration'}"
                            )
